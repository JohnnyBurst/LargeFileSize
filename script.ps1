...
$largesizefiles = Get-ChildItem _path $path -recurse -ErrorAction "SilentlyContinue" | ? { $_.GetType().Name -eq "FileInfo" } | where-object {$_.length -gt $size} | sort-object -property length -descending | select-object @{Name="SizeInMB";Expression={$_.Length / 1MB}},@{Name="Path";Expression={$_.directory}} -first $limit
